...
function createLanguagePassthroughModule (file) {
  ...
  fs.writeFile(path.join(__dirname, `../src/languages/lowlight/${file}`), lines.join(';\n'));
}
...
fs.readdir(path.join(__dirname, '../node_modules/highlight.js/lib/languages'), (err, files) => {
  ...
  fs.writeFile(path.join(__dirname, '../AVAILABLE_LANGUAGES_LOWLIGHT.MD'), languageMD);

  const defaultExports = availableLanguageNames.map((name) => `export { default as ${makeImportName(name)} } from './${name}';\n`);
  fs.writeFile(path.join(__dirname, '../src/languages/lowlight/index.js'), defaultExports.join(''));
});