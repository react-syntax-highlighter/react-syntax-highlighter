...
fs.readdir(path.join(__dirname, '../node_modules/highlight.js/styles'), (err, files) => {
  ...
  fs.writeFile(path.join(__dirname, '../AVAILABLE_STYLES_LOWLIGHT.MD'), styleMD);
  fs.writeFile(path.join(__dirname, '../src/styles/lowlight/index.js'), defaultExports.join(''));
});
...
function createJavascriptStyleSheet(file) {
  ...
  fs.readFile(path.join(__dirname, `../node_modules/highlight.js/styles/${file}`), 'utf-8', (err, data) => {
    ...
    fs.writeFile(path.join(__dirname, `../src/styles/lowlight/${fileWithoutCSS}.js`), 
